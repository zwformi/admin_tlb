...
<body>
    ...
    <script type="text/javascript">
    	...
    	$(document).ready(function () {
            ...
            $("#user_id").val(getUrlParam("id"));

    		$.post("<%=basePath%>salesman/query2.json",{"id":getUrlParam("id")},function(data){
				if(data == null || data.name == undefined){
	    			$("#tip").html("<span style='color:red;font-size:14px'>未分配销售</span>");
				}else{
					$("#tip").html("<span style='color:green;font-size:14px'>已分配专属销售："+data.name+"，工号："+data.gh+"，电话："+data.phone+"，QQ号码："+data.qq+"</span>");
				}
    		});
            ...
            $("#save").click(function(){
            	if($("input[name='check']:checked").length == 0){
            		layer.alert("请选择销售人员", {icon: 7});
            	}else if($("input[name='check']:checked").length == 1){
            		var id = $("input[name='check']:checked").val();

                    $.post("<%=basePath%>user/modify3.json",{"user_id":$("#user_id").val(),"sales_id":id},function(data){
            			if(data.success){
