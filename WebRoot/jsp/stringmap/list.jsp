...
<body>
	<div class="place">
        ...
        <ul class="placeul">
            ...
            <li><a href="javascript:;">生产字典管理</a></li>
        </ul>
    </div>
    
    <div class="formbody">
        <div class="formtitle"><span>生产字典列表</span></div>
        <div class="rightinfo">
            <div class="tools">
                <ul class="toolbar">
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_stringmap_add")){%>
                	<li id="add"><span><img src="<%=basePath%>images/t01.png" /></span>新增</li>
                <%}%>
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_stringmap_edit")){%>
                    <li class="edit"><span><img src="<%=basePath%>images/t02.png" /></span>编辑</li>
                <%}%>
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_stringmap_delete")){%>
                    <li class="delete"><span><img src="<%=basePath%>images/t03.png" /></span>删除</li>
                <%}%>
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_stringmap_search")){%>
            ...
            <table class="tablelist">
    			...
    			<tr>
					<td style="width:10px;"><input name="check" type="checkbox" value="${id}" /></td>
    				<td style="width:100px;">${name}</td>
    				<td style="width:100px;">${pinyin}</td>
					<td style="width:50px;">${value}</td>
    				<td style="width:100px;">${objectTypeName}</td>
    				<td style="width:100px;">${filedName}</td>
    			</tr>
    			</script>
            </table>
    ...
    <script type="text/javascript">
            ...
            $(".delete").click(function(){
            	if($("input[name='check']:checked").length != 0){
            		layer.confirm('确认删除信息 ？', {icon: 3}, function(index){
						...
						$.post("<%=basePath%>stringmap/delete.json",{"ids":ids},function(data){
				    		layer.close(index);
				    		initData();
        ...
        function initData(){
            ...
            $.post("<%=basePath%>stringmap/count.json",{"keyword":keyword},function(data){
        ...
        function pageSelectCallback(index, jq) {
        	$("#index").html(index+1);
        	var keyword = $(".dfinput").val();
        	$.post("<%=basePath%>stringmap/paginglist.json",{"keyword":keyword,"pageindex":index,"pagesize":pageSize,"orderparam":"ObjectTypeName,FiledName"},function(data){
	            if(data.data!=null)
	            $("#content").html($("#template").tmpl(data.data));
	            else
	             $("#content").html('');
	            //加载表格样式,公用
	            loadTableStyle();
