...
<form id="mForm">
    ...
    <span>位置：</span>
        <ul class="placeul">
            ...
            <li><a href="<%=basePath%>jsp/salesman/list.jsp">专属销售管理</a></li>
            <li><a href="javascript:;" class="location_add">新增销售人员</a></li>
        </ul>
    </div>
    
    <div class="formbody">
    <div class="formtitle"><span class="title_add">新增销售人员</span></div>
        <ul class="forminfo">
        	<li><label>销售员工号：</label>
                <dl><input id="gh" name="gh" type="text" class="dfinput" datatype="*2-50" nullmsg="请填写销售员工号" /></dl>
            </li>
            <li><label>销售姓名：</label>
                <dl><input id="name" name="name" type="text" class="dfinput" datatype="*2-20" nullmsg="请填写销售姓名" /></dl>
            </li>
            <li><label>联系电话：</label>
...
$(function() {
	...
	if(action == "edit"){
		$("#id").val(id);
		$("#action").val(action);
		$(".location_add").html("编辑销售人员");
		$(".title_add").html("编辑销售人员");
		
		$.post("<%=basePath%>salesman/query.json",{"id":id},function(data){
			$("#gh").val(data.gh);
			$("#name").val(data.name);
...
$(".btn").click(function(){
	if(checkValidform.check()){
		var url = "<%=basePath%>salesman/add.json";
		if($("#action").val() == "edit"){
			url = "<%=basePath%>salesman/modify.json";
		}
		var frm=$('#mForm').serialize();
		$.post(url,frm,function(data){
			if(data.success){
				layer.confirm('保存成功！是否返回列表页？', {icon: 1}, function(index){
					location.href="<%=basePath%>jsp/salesman/list.jsp";
				});
				if($("#action").val() != "edit"){
					$('#mForm')[0].reset();
				}
