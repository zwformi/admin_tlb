$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "jpg,png,jpeg,gif,doc,docx,xls,xlsx,ppt,htm,html,txt,zip,rar,gz,bz2,pdf", //文件类型
        filesize: "4096", //文件大小
        btntext: "浏览...", //上传按钮的文字
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            //初始化属性
            p.sendurl += "?dir=file&action=UpLoadFile&order_id="+$("#order_id").val();
            if (p.water) {
                p.sendurl += "&IsWater=1";
            }
            if (p.thumbnail) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        var file_html2 = "<a href=\""+data.path+"\" target=\"_blank\" style=\"color:blue;\">查看合同附件</a>";
                        $("#demand_file").html(file_html2);
                        parent.layer.msg("合同附件上传成功");
                    }
                    var progressObj = parentObj.children(".upload-progress");
