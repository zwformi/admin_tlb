...
<body>
    ...
    <div class="formbody">
        <div class="formtitle"><span>品牌列表</span></div>
        <div class="rightinfo">
            <div class="tools">
                <ul class="toolbar">
                	<%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_product_brand_add")){%>
                	<li id="add"><span><img src="<%=basePath%>images/t01.png" /></span>新增</li>
                	<%}%>
                	<%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_product_brand_edit")){%>
                    <li class="edit"><span><img src="<%=basePath%>images/t02.png" /></span>编辑</li>
                    <%}%>
                	<%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_product_brand_delete")){%>
                    <li class="delete"><span><img src="<%=basePath%>images/t03.png" /></span>删除</li>
                    <%}%>
                	<%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_product_brand_search")){%>
    ...
    <script type="text/javascript">
            ...
            $(".delete").click(function(){
            	if($("input[name='check']:checked").length != 0){
            		layer.confirm('确认删除信息 ？', {icon: 3}, function(index){
						...
						$.post("<%=basePath%>brand/delete.json",{"id":id},function(data){
				    		layer.close(index);
				    		initData();
				        });
					});
        ...
        function initData(){
            ...
            $.post("<%=basePath%>brand/queryCount.json",{"keyword":keyword},function(data){
        ...
        function pageSelectCallback(index, jq) {
        	$("#index").html(index+1);
        	var keyword = $(".dfinput").val();
        	$.post("<%=basePath%>brand/queryList.json",{"keyword":keyword,"pageIndex":index,"pageSize":pageSize},function(data){
