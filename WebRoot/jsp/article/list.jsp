...
<body>
	<div class="place">
        ...
        <ul class="placeul">
            ...
            <li><a href="javascript:;">单页管理</a></li>
        </ul>
    </div>
    
    <div class="formbody">
        <div class="formtitle"><span>单页列表</span></div>
        <div class="rightinfo">
            <div class="tools">
                <ul class="toolbar">
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_article_add")){%>
                	<li id="add"><span><img src="<%=basePath%>images/t01.png" /></span>新增</li>
                <%}%>
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_article_edit")){%>
                    <li class="edit"><span><img src="<%=basePath%>images/t02.png" /></span>编辑</li>
                <%}%>
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_article_delete")){%>
                    <li class="delete"><span><img src="<%=basePath%>images/t03.png" /></span>删除</li>
                <%}%>
                <%if(com.yunrer.common.AuthUtil.getAttribute(session, "b_article_search")){%>
            ...
            <table class="tablelist">
    			...
    			<tr>
    				...
    				<td>
						{{if typeid == 1}}产品服务{{/if}}
						{{if typeid == 3}}解决方案{{/if}}
						{{if typeid == 2}}关于图灵买{{/if}}
						{{if typeid == 99}}公司大事记{{/if}}
						{{if typeid == 4}}其他{{/if}}
					</td>
    			</tr>
    			</script>
    ...
    <script type="text/javascript">
            ...
            $(".delete").click(function(){
            	if($("input[name='check']:checked").length != 0){
            		layer.confirm('确认删除信息 ？', {icon: 3}, function(index){
						...
						$.post("<%=basePath%>article/delete.json",{"id":id},function(data){
				    		layer.close(index);
				    		initData();
				        });
					});
        ...
        function initData(){
            ...
            $.post("<%=basePath%>article/queryCount.json",{"keyword":keyword},function(data){
        ...
        function pageSelectCallback(index, jq) {
        	$("#index").html(index+1);
        	var keyword = $(".dfinput").val();
        	$.post("<%=basePath%>article/queryList.json",{"keyword":keyword,"pageIndex":index,"pageSize":pageSize},function(data){
